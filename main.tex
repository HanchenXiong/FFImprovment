...
%------------------------------------------------
\section{Online game playing, portfolio and bandit}        % Sections can be created in order to organize your presentation into discrete blocks, all sections and subsections are automatically printed in the table of contents as an overview of the talk
\subsection{Prediction with Experts' advice}
\frame{
    \frametitle{A gentle start} 
    \begin{itemize}
    \item<1-> \structure{Task:}  online prediction of a binary sequence, \emph{i.e.} sequentially forecast a value $y_t\in\{0,1\}$ at time $t$ based on historical values $\{y_i\}_{i=1}^t$;
    \item<2->  \structure{Exert committee:} an expert $E_{n}$ is a man/woman who can make a prediction, $f_{n,t}$, using different strategies (algorithms/heuristics/data resources); assume there are $N$ experts in the committee; 
    \item<3->  \structure{Side information:} other task-relevant information may available, \emph{e.g.} $\{\mathbf{x}_i\}_{i=1}^{t}$;
    ...
    \item<5->  \structure{Weights update:} at the beginning, each expert is assigned a (uniform) weight $w_{n,0}$, and it will be updated along the sequence; 
    \item<6->  \structure{Loss:}  $l(\hat{y}_t, y_t) = \mathbf{1}_{\hat{y}_t \neq y_t}$, $l(f_{n,t}, y_t) = \mathbf{1}_{f_{n,t} \neq y_t}$
    \end{itemize}
}
...
\frame{
    ...
    The final decision is made with the \alert{majority voting} from the expert committee: $\hat{y}_t = \textbf{sign} (\frac{\sum_{n=1}^N f_{i,t}}{N} - 0.5 )$;
    \end{block}}
    ...
    \begin{equation}
     \hat{L}_t = \sum_{i=t}^{t}l(\hat{y}_t, y_t) \leq \log_{2}N; 
    \end{equation}
    }
}
...
\frame{
    ...
    \begin{equation}
     R_{n,t}= \hat{L}_t - L_{n,t} = \sum_{i=t}^{t}l(\hat{y}_t, y_t) - \sum_{i=t}^{t}l(f_{n,t}, y_t)
    \end{equation}
    }

    \onslide<+->{\begin{block}{the upper bound of regret}
    $R_{t}^* = \max_{n\in[1,N]} R_{n,t} = \hat{L}_t - \min_{n\in[1,N]}L_{n,t}$
    \end{block}}
}
    

\frame{
    \frametitle{Multiplicative weights majority}
    \onslide<+->{\begin{block}{A more realistic scenario}
     we know that in the committee there exists a best expert who can work better than others in the given task;
    \end{block}}
    \onslide<+->{
...
\frame{
 ...
 \onslide<+->One player plays a game, at time $t$ he takes one action $a_t = i\in[1,N]$, then the environment penalize the player 
 with a loss function $l_t(a_t)$. 

 \onslide<+->{\alert{Note} that the loss function $l_t$ can change over time, i.e. the environment is changing.}
}
